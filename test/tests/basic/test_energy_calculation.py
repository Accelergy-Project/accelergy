...
class TestEnergyCalculation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.desired_ERT_dict = {'design.mac': {'mac_random': [{'name': 'mac_random', 'arguments': {}, 'energy': 3}],
                                                'mac_reused': [{'name': 'mac_reused', 'arguments': {}, 'energy': 2}],
                                                'mac_gated': [{'name': 'mac_gated', 'arguments': {}, 'energy': 1}],
                                                'leak': [{'name': 'leak', 'arguments': {}, 'energy': 0}]
                                                },
        ...
        self.desired_action_counts_dict = {'design.mac': [{'name': 'mac_random', 'counts': 50},
                                           ...
                                           'design.scratchpad[0]': [{'name': 'leak', 'counts': 100},
                                                                    {'name': 'fill',
                                                                    'arguments': {'address_delta': 0, 'data_delta': 0},
                                                                     'counts': 1150},
                                                                    {'name': 'fill',
    ...
    def test_fromYAMLfileToERTDict(self):
        ...
        raw_input_info = {'path_arglist': [
            './data/ERT.yaml'], 'parser_version': self.version}
    ...
    def test_fromYAMLfileToActionCountsDict(self):
        ...
        raw_input_info = {'path_arglist': [
            './data/action_counts.yaml'], 'parser_version': self.version}
        raw_dicts = RawInputs2Dicts(raw_input_info)
    ...
    def test_energyComputation(self):
        ...
        desired_action_counts_obj = action_counts_dict_2_obj(
            self.desired_action_counts_dict)
        energy_calculator = EnergyCalculator({'action_counts': desired_action_counts_obj,
                                              'ERT': desired_ERT_obj,
                                              'parser_version': self.version})

        mac_energy = energy_calculator.energy_estimates.get_energy_estimation(
            'design.mac')
        scrachpad_energy = energy_calculator.energy_estimates.get_energy_estimation(
            'design.scratchpad[0]')
    ...
    def test_wrongActionCounts(self):
        ...
        desired_ERT_obj = ERT_dict_to_obj(
            {'ERT_dict': self.desired_ERT_dict, 'parser_version': self.version, 'precision': 3})
        raw_input_info = {'path_arglist': [
            './data/action_counts_error.yaml'], 'parser_version': self.version}
        raw_dicts = RawInputs2Dicts(raw_input_info)
