...
class Test(AccelergyUnitTest):
    ...
    def test_area_share(self):
        ...
        self.assert_area('arch.three_level_hierarchy_with_area_share', 2 * 4 * 3 * 2)
        self.assert_area('arch.two_level_hierarchy_with_area_share', 3 * 3 * 2)
        self.assert_area('arch.one_level_hierarchy_with_area_share', 4 * 2)
        self.assert_area('arch.primitive_direct_from_arch_with_area_share', 5)

        # Hierarchy area with area share in the arch spec but not in attributes
        self.assert_area('arch.three_level_hierarchy_with_area_share_defined_not_in_attrs', \
            6 * 4 * 3 * 2)
        self.assert_area('arch.two_level_hierarchy_with_area_share_defined_not_in_attrs', \
            7 * 3 * 2)
        self.assert_area('arch.one_level_hierarchy_with_area_share_defined_not_in_attrs', \
            8 * 2)
        self.assert_area('arch.primitive_direct_from_arch_with_area_share_defined_not_in_attrs', \
            9)
    ...
    def test_action_share(self):
        ...
        self.assert_energy('arch.two_subcomponents_different_shares', 'write', 0.5 + 0.25)